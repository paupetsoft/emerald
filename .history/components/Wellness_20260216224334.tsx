...
export default function Wellness() {
  ...
  const { scrollYProgress } = useScroll({
    target: targetRef,
    // "end center" means the progress reaches 100% when the section bottom hits screen middle
    offset: ["start start", "end center"]
  });

  // Mapping the movement to finish slightly before the scroll ends (0.95)
  // this prevents the "stuck" feeling at the final image.
  const x = useTransform(scrollYProgress, [0, 0.95], ["0%", "-400%"]);

  return (
    /* h-[350vh] is the sweet spot for these 5 panels with the new offset */
    <section ref={targetRef} className="relative h-[350vh] bg-[#050a08]">
