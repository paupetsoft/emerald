...
export default function Wellness() {
  ...
  // Ending at 0.85 (85%) ensures the animation is DONE long before the user hits the bottom
  const x = useTransform(scrollYProgress, [0, 0.85], ["0%", "-400%"]);
          ...
          {spaImages.map((item, i) => (
            <div key={i} className="w-screen h-screen flex-shrink-0 flex items-center justify-center">
              <div className="relative w-[85vw] h-[75vh]">
                <img 
                  src={item.url} 
                  className="w-full h-full object-cover brightness-50 border border-white/10" 
                  alt={item.title} 
                />
